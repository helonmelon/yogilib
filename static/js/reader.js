(() => {
  ...
  async function api(path, method='GET', data) {
    ...
    if (!r.ok || r.redirected) throw Error('Could not complete this action. Check your connection and sign-in, then try again.');
  ...
  if(body) {
    ...
    body.querySelectorAll('h1,h2,h3,h4,p,li').forEach(el=>{
      ...
      const a=document.createElement('a');a.href='#'+el.id;a.className='passage-link';a.textContent='¶';a.setAttribute('aria-label','Copy link to this passage');
      a.addEventListener('click',async e=>{e.preventDefault();const url=new URL(location.href);url.hash=el.id;history.replaceState(null,'',url);try{await navigator.clipboard.writeText(url.href);status('Passage link copied.');}catch{status('Copy the passage link from your address bar.');}});el.append(a);
    });
    if(!$('reader-toc').children.length) $('reader-toc').textContent='No headings in this document yet.';
  ...
  $('reader-search').addEventListener('submit',e=>{e.preventDefault();matches=rangesFor($('reader-query').value);active=-1;if(window.Highlight&&CSS.highlights){CSS.highlights.delete('reader-active');CSS.highlights.set('reader-search',new Highlight(...matches));}$('search-count').textContent=matches.length?'':'No matches';focusMatch(1);});
  $('search-prev').onclick=()=>focusMatch(-1);$('search-next').onclick=()=>focusMatch(1);
  const original=$('reader-original');if(original){const file=original.dataset.file;if(/\.(png|jpe?g|gif|webp|avif)(\?|$)/i.test(file)){const img=document.createElement('img');img.src=file;img.alt='Original document scan';original.querySelector('object').replaceWith(img);} $('reader-split').onclick=()=>{const on=document.querySelector('.reader-columns').classList.toggle('split');$('reader-split').setAttribute('aria-pressed',on);};}
  let quote='';
  if($('capture-selection')) {
    ...
    $('capture-selection').onclick=()=>status(quote?'Passage selected. Add an optional note and save.':'Select text in the document first.');
    async function loadNotes(){const notes=await api('/notes');$('notes-list').replaceChildren();const highlights=[];notes.forEach(n=>{highlights.push(...rangesFor(n.quote));const card=document.createElement('div');card.className='reader-note-card';const q=document.createElement('blockquote');q.textContent=n.quote;const p=document.createElement('p');p.textContent=n.note;const jump=document.createElement('button');jump.textContent='Find passage';jump.onclick=()=>{matches=rangesFor(n.quote);active=-1;if(matches.length)focusMatch(1);else status('This passage has changed since the note was saved.');};const remove=document.createElement('button');remove.textContent='Delete note';remove.onclick=async()=>{try{await api('/notes/'+n.id,'DELETE');await loadNotes();}catch(e){status(e.message);}};card.append(q,p,jump,remove);$('notes-list').append(card);});if(window.Highlight&&CSS.highlights)CSS.highlights.set('reader-notes',new Highlight(...highlights));if(!notes.length)$('notes-list').textContent='No saved highlights yet.';}
    $('save-note').onclick=async()=>{if(!quote){status('Select a passage first.');return;}try{await api('/notes','POST',{quote,note:$('reader-note').value});$('reader-note').value='';await loadNotes();status('Saved privately to your account.');}catch(e){status(e.message);}};
    loadNotes().catch(e=>status(e.message));
  }
  $('reader-history')?.addEventListener('toggle',async()=>{if(!$('reader-history').open)return;try{const revisions=await api('/revisions');$('history-list').replaceChildren();for(const v of revisions){const d=document.createElement('details'),summary=document.createElement('summary'),pre=document.createElement('p'),button=document.createElement('button');summary.textContent=`${v.date} — ${v.title}`;pre.textContent=v.text;pre.style.whiteSpace='pre-wrap';button.textContent='Restore this version';button.onclick=async()=>{if(!confirm('Restore this version? The current version will be saved in history.'))return;try{await api('/revisions/'+v.id,'POST');location.reload();}catch(e){status(e.message);}};d.append(summary,pre,button);$('history-list').append(d);}if(!revisions.length)$('history-list').textContent='No previous versions yet.';}catch(e){status(e.message);}});
})();
